...
class BenchmarkMonitor:
    ...
    def end_benchmark(self, additional_metrics: Optional[Dict[str, float]] = None) -> BenchmarkResult:
    ...
    def _save_result(self, result: BenchmarkResult) -> None:
        ...
        result_file = self.results_dir / f"{result.test_name}_{result.timestamp:%Y%m%d_%H%M%S}.json"
    ...
    def _load_historical_results(self, test_name: str) -> List[BenchmarkResult]:
        ...
        results = []
        result_files = sorted(
            self.results_dir.glob(f"{test_name}_*.json"),
            reverse=True
        )[:self.history_length]
    ...
    def generate_report(self) -> Dict[str, Dict[str, float]]:
        ...
        for result in self.current_results:
            if result.test_name not in report:
                ...
                stats['mean_time'] = (stats['mean_time'] * stats['count'] + result.execution_time) / (stats['count'] + 1)
                stats['mean_memory'] = (stats['mean_memory'] * stats['count'] + result.memory_usage) / (stats['count'] + 1)
                stats['mean_cpu'] = (stats['mean_cpu'] * stats['count'] + result.cpu_utilization) / (stats['count'] + 1)
                stats['count'] += 1
