...
from src.utils.test_helpers import assert_pattern_properties
...
class TestPatternDynamics:
    ...
    @pytest.fixture
    def pattern_system(self, space_dim, grid_size):
    ...
    def test_reaction_diffusion(self, pattern_system, grid_size, batch_size):
        ...
        assert pattern_system.detect_pattern_formation(time_evolution), \
            "Should form stable patterns"

    def test_stability_analysis(self, pattern_system, grid_size):
    ...
    def test_bifurcation_analysis(self, pattern_system, grid_size):
        ...
        for point in diagram.bifurcation_points:
            assert point.type in ['saddle-node', 'hopf', 'pitchfork'], \
                "Should classify bifurcation type"
                
        # Test normal form computation
        normal_form = pattern_system.compute_normal_form(
            diagram.bifurcation_points[0]
        )
        assert normal_form is not None, "Should compute normal form"

    def test_pattern_control(self, pattern_system, grid_size):
    ...
    def test_spatiotemporal_evolution(self, pattern_system, grid_size):
        ...
        assert pattern_type in [
            'stationary', 'periodic', 'quasi-periodic', 'chaotic'
    ...
    def test_reaction_diffusion_dynamics(
        ...
        def test_diffusion(state, dt=0.01):
            ...
            assert torch.norm(
                torch.diff(diffused, dim=1)
            ) < torch.norm(
                torch.diff(state, dim=1)
            ), "Diffusion should smooth gradients"
            return diffused
            
        diffused_state = test_diffusion(state)
        ...
        reacted_state = test_reaction(state)
        ...
        def test_full_dynamics(state, steps=100):
            ...
            assert trajectory.shape == (
                steps, batch_size, grid_size, grid_size, 2
        ...
        trajectory = test_full_dynamics(state)
        
        # Test Turing instability
        def test_turing_instability():
            ...
            assert torch.any(
                dispersion > 0
            ), "Should have positive growth rates"
    ...
    def test_stability_analysis(
        ...
        eigenvals = test_linear_stability()
        ...
        def test_nonlinear_stability(state, time=10.0):
            ...
            lyapunov = pattern_system.compute_lyapunov_function(
        ...
        lyapunov = test_nonlinear_stability(state)
        ...
        def test_basin_of_attraction():
            ...
            final_state = pattern_system.evolve_pattern(
                state_in_basin, 1000
            )[-1]
            assert torch.allclose(
    ...
    def test_bifurcation_analysis(
        ...
        def test_symmetry_breaking():
            ...
            broken_symm = pattern_system.analyze_symmetry_breaking(
                bifurcations
            )
    ...
    def test_control_systems(
        ...
        target = pattern_system.generate_target_pattern(
        ...
        control_matrix = test_controllability()
        
        # Test optimal control
        def test_optimal_control(target, time=1.0):
            """Test optimal control computation."""
            control = pattern_system.compute_optimal_control(
                target, final_time=time
            )
            assert control.shape[1] == pattern_system.n_controls, \
        ...
        control = test_optimal_control(target)
        ...
        def test_feedback_control():
            ...
            closed_loop_matrix = pattern_system.compute_closed_loop_matrix(
                feedback_law
            )
        ...
        feedback_law = test_feedback_control()
        ...
        def test_robustness():
            ...
            assert robust_control.shape[1] == pattern_system.n_controls, \
                "Should have correct control dimensions"
