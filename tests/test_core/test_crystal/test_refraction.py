...
class TestRefractionSystem:
    ...
    def refraction_system(self, space_dim, lattice_points):
        ...
        return RefractionSystem(
            dim=space_dim, size=lattice_points, symmetry_tolerance=1e-5
        )
    ...
    def test_symmetry_computation(self, refraction_system, space_dim):
        ...
        assert (
            len(symmetry_group.generators) == 3
        ), "Cubic group should have 3 generators"
        ...
        assert all(
            test_closure(g1, g2) for g1, g2 in product(generators, repeat=2)
    ...
    def test_lattice_detection(self, refraction_system, space_dim):
        ...
        assert (
            len(lattice.basis) == space_dim
        ), "Should have correct number of basis vectors"
    ...
    def test_brillouin_zones(self, refraction_system, space_dim):
        ...
        assert is_in_first_zone(
            torch.zeros(space_dim)
        ), "Origin should be in first zone"
    ...
    def test_phonon_modes(self, refraction_system, space_dim):
        ...
        assert (
            len(acoustic_modes) == space_dim
        ), "Should have correct number of acoustic modes"
        ...
        def test_debye_scaling(k):
            ...
            return all(
                torch.allclose(
                    mode.frequency(k), mode.sound_velocity() * torch.norm(k), rtol=1e-2
                )
                for mode in acoustic_modes
            )
    ...
    def test_optical_properties(self, refraction_system, space_dim):
        ...
        def test_dispersion(k_vector):
            ...
            return torch.allclose(
                omega**2, torch.dot(k_vector, k_vector) / epsilon[0, 0], rtol=1e-4
            )
        ...
        k_vector = torch.tensor([1.0, 0.0, 1.0]) / np.sqrt(2)
        ...
        assert (
            torch.abs(rays[0].polarization.dot(rays[1].polarization)) < 1e-4
        ), "Rays should be orthogonally polarized"
    ...
    def test_thermal_transport(self, refraction_system, space_dim, lattice_points):
        ...
        assert torch.all(
            eigenvals > 0
        ), "Thermal conductivity should be positive definite"

        # Test Fourier's law
        heat_flux = -kappa @ torch.tensor([0.0, 0.0, (T_hot - T_cold) / lattice_points])
    ...
    def test_strain_effects(self, refraction_system, space_dim):
        ...
        eigenvals = torch.linalg.eigvalsh(C.reshape(space_dim**2, space_dim**2))
        ...
        # Grüneisen parameter
        gamma = -(strained_phonons.frequencies - unstrained_phonons.frequencies) / (
