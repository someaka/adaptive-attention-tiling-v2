...
from src.utils.test_helpers import assert_tensor_properties
...
class TestRiemannianStructure:
    ...
    def test_metric_tensor(self, riemannian_structure, manifold_dim):
        ...
        assert riemannian_structure.metric_tensor(
            point, (v, v)
        ) >= 0, "Metric should be positive definite"
    ...
    def test_christoffel_symbols(self, riemannian_structure, manifold_dim):
        ...
        # Test shape
        assert christoffel.shape == (manifold_dim, manifold_dim, manifold_dim), \
            "Christoffel symbols should have correct shape"
            
        # Test symmetry in lower indices
        assert torch.allclose(
            christoffel.transpose(-1, -2),
            christoffel,
            rtol=1e-5
    ...
    def test_curvature_tensor(self, riemannian_structure, manifold_dim):
        ...
        # Test shape
        assert R.shape == (manifold_dim,) * 4, "Curvature tensor should have correct shape"
        ...
        assert torch.allclose(bianchi1, torch.zeros_like(R), rtol=1e-5), \
        ...
        assert torch.allclose(bianchi2, torch.zeros_like(nabla_R), rtol=1e-5), \
    ...
    def test_killing_fields(self, riemannian_structure, manifold_dim):
        ...
        is_killing = torch.allclose(lie_derivative, torch.zeros_like(lie_derivative), rtol=1e-5)
    ...
    def test_metric_properties(
        self, riemannian_manifold, batch_size, manifold_dim
    ):
        ...
        def test_compatibility():
            ...
            nabla_g = riemannian_manifold.covariant_derivative(
                lambda v: riemannian_manifold.inner_product(Y(v), Z(v)),
                X
            )
    ...
    def test_curvature_tensors(
        ...
        def test_ricci():
            ...
            # Test second Bianchi identity
            nabla = riemannian_manifold.get_levi_civita()
            bianchi2 = riemannian_manifold.covariant_derivative(
                Ric, nabla
            )
            assert torch.allclose(
                torch.trace(bianchi2),
                0.5 * riemannian_manifold.gradient(
                    riemannian_manifold.scalar_curvature()
                )
    ...
    def test_geodesics(
        ...
        def test_exponential():
            ...
            # Test exp(0) = id
            assert torch.allclose(
                riemannian_manifold.exponential_map(
        ...
        def test_jacobi():
            ...
            jacobi_op = (
                riemannian_manifold.covariant_derivative(
                    lambda t: riemannian_manifold.covariant_derivative(
                        J, gamma.velocity
                    )(t),
                    gamma.velocity
                ) +
