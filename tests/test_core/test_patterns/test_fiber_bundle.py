...
class TestFiberBundle:
    ...
    def test_associated_bundles(self, fiber_bundle, structure_group):
        ...
        point = torch.randn(4, 11)
        torch.randn(4, 2)  # Vector in associated bundle
    ...
    def test_bundle_parallel_transport(self, fiber_bundle, base_manifold, fiber_dim):
        """Test parallel transport in the context of bundle structure."""
        # Test parallel transport properties
        def test_transport():
            """Test parallel transport along paths."""
            # Create a path in base manifold
            t = torch.linspace(0, 2 * np.pi, 100)
            path = torch.stack([torch.cos(t), torch.sin(t)], dim=1)
            
            # Create initial frame
            initial_frame = torch.eye(fiber_dim)
            
            # Transport frame
            transported = fiber_bundle.parallel_transport(initial_frame, path)
            
            # Test transport properties
            assert transported.shape[-1] == fiber_dim, "Should preserve fiber dimension"
            assert fiber_bundle.is_parallel(transported), "Should be parallel"
        ...
        # Test holonomy
        def test_holonomy():
            """Test holonomy of parallel transport."""
            # Get holonomy
            holonomy = fiber_bundle.get_holonomy(transported)
            
            # Test holonomy properties
            assert torch.allclose(
                holonomy @ holonomy.transpose(-1, -2),
                torch.eye(fiber_dim),
                rtol=1e-5
