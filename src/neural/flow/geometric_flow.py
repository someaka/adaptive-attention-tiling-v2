...
class FlowStepNetwork(nn.Module):
    ...
    def __init__(self, manifold_dim: int, hidden_dim: int = 64):
        super().__init__()
        self.manifold_dim = manifold_dim
        ...
        self.energy_network = nn.Sequential(
            nn.Linear(manifold_dim * 2, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 1)
        )

    def compute_flow_vector(
        self, points: torch.Tensor, ricci: torch.Tensor
    ) -> torch.Tensor:
        """Compute flow vector field."""
        batch_size = points.shape[0]
        input_tensor = torch.cat([points, ricci.reshape(batch_size, -1)], dim=1)
...
class FlowNormalizer(nn.Module):
    ...
    def normalize_flow(self, flow: torch.Tensor, energy: torch.Tensor) -> torch.Tensor:
