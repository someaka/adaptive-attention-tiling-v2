...
from .geometric_flow import GeometricFlow, FlowMetrics
...
class HamiltonianNetwork(nn.Module):
    ...
    def forward(
        ...
        phase_vector = torch.cat([
            phase_point.position,
            phase_point.momentum
        ], dim=-1)
...
class PoissonBracket(nn.Module):
    ...
    def compute_bracket(
        ...
        # Compute gradients
        point_vector = torch.cat([
            point.position,
            point.momentum
        ], dim=-1)
        point_vector.requires_grad_(True)
        
        f_val = f(PhaseSpacePoint(
            point_vector[:, :self.phase_dim],
            point_vector[:, self.phase_dim:],
            point.time
        ))
        g_val = g(PhaseSpacePoint(
            point_vector[:, :self.phase_dim],
            point_vector[:, self.phase_dim:],
            point.time
        ))
...
class ConservationLaws(nn.Module):
    ...
    def track_conservation(
        self,
        quantities: List[ConservedQuantity],
...
class HamiltonianSystem:
    ...
    def evolve(
        ...
        for _ in range(num_steps):
            ...
            H = self.hamiltonian(PhaseSpacePoint(
            ...
            # Track conservation
            quantities = self.conservation.track_conservation(
                quantities, new_point
            )
