...
class PatternProcessor(nn.Module):
    ...
    def process_pattern(
        ...
        """
        print("Pattern Processor Steps:")
        print(f"Input pattern shape: {pattern.shape}")
        
        # Project to hidden dimension
        hidden_pattern = self.pattern_net(pattern)
        print(f"After initial projection shape: {hidden_pattern.shape}")
        
        # Pad to total dimension (base_dim + fiber_dim)
        padded_pattern = torch.zeros(*hidden_pattern.shape[:-1], self.pattern_bundle.total_dim,
                                   device=pattern.device, dtype=pattern.dtype)
        padded_pattern[..., :self.manifold_dim] = pattern  # Copy base manifold components
        padded_pattern[..., self.manifold_dim:self.manifold_dim + self.hidden_dim] = hidden_pattern  # Copy fiber components
        print(f"After padding shape: {padded_pattern.shape}")
        
        # Now project through bundle
        bundle_point = self.pattern_bundle.bundle_projection(padded_pattern)
        
        if return_intermediates:
            intermediates = {
                "hidden_pattern": hidden_pattern,
                "padded_pattern": padded_pattern,
                "bundle_point": bundle_point
            }
            return bundle_point, intermediates
