...
from typing import TypeVar, Generic, Protocol, Tuple, List, Optional
...
class PatternRiemannianStructure(nn.Module):
    ...
    def metric_tensor(
        ...
        v1, v2 = vectors
        return torch.sum(
            torch.matmul(metric, v1.unsqueeze(-1)) * v2.unsqueeze(-1),
            dim=-1
        )
    
    def christoffel_symbols(self, chart: torch.Tensor) -> ChristoffelSymbols[torch.Tensor]:
        """Computes Christoffel symbols in a chart."""
    ...
    def curvature_tensor(self, point: torch.Tensor) -> CurvatureTensor[torch.Tensor]:
        ...
        gamma_grad = torch.autograd.grad(
            gamma.sum(), point,
            create_graph=True
        )[0]
