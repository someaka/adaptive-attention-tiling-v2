...
from typing import TypeVar, Generic, Protocol, Tuple, List
...
class PatternFiberBundle(nn.Module):
    ...
    def __init__(
        ...
        self.metric = nn.Parameter(
            torch.eye(base_dim, device=self.device).unsqueeze(0)
        )
    ...
    def transition_functions(
        ...
        return torch.eye(self.fiber_dim, device=self.device) + \
               self.connection.matmul(diff.unsqueeze(-1)).squeeze(-1)
    ...
    def parallel_transport(
        ...
        for t in range(path.size(-2) - 1):
            result = result + connection_values[..., t, :, :].matmul(result.unsqueeze(-1)).squeeze(-1)
