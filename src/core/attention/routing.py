...
class InformationRouter(nn.Module):
    ...
    def predict_optimal_path(
        ...
        for t in range(seq_len - 1):
            ...
            out, hidden = self.path_predictor(
                current_state.view(-1, 1, self.dim),
                hidden
            )
            
            # Compute scores for all positions
            scores = self.route_scorer(
                states.view(-1, seq_len, self.dim)
            ).view(batch, heads, seq_len)
    ...
    def forward(
        ...
        pattern_info = self.pattern_dynamics(
            states,
        ...
        bottlenecks, impact = self.detect_bottlenecks(
            states,
            routing_scores
        )
        
        # Optimize states
        optimized_states = self.optimize_bottlenecks(
            states,
            bottlenecks,
            impact
        )
        ...
        optimal_paths = self.predict_optimal_path(
            optimized_states,
            start_idx
        )
