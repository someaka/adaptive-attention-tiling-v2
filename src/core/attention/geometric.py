...
class GeometricStructures(nn.Module):
    ...
    def __init__(
        ...
        self.transport = ParallelTransport(
            dim,
            method=parallel_transport_method
        )
    ...
    def compute_sectional_curvature(
        ...
        g11 = torch.einsum('i,ij,j->', v1, self.metric, v1)
        g12 = torch.einsum('i,ij,j->', v1, self.metric, v2)
        g22 = torch.einsum('i,ij,j->', v2, self.metric, v2)
        
        area = torch.sqrt(g11 * g22 - g12 * g12)
    ...
    def compute_geodesic_distance(
        ...
        return torch.sqrt(
            torch.einsum('i,ij,j->', v, self.metric, v)
        )
    ...
    def forward(
        ...
        if v is not None:
            # Parallel transport
            transported = self.parallel_transport_batch(x, y, v)
            results['transported'] = transported
            
            # Geodesic
            curve = self.compute_geodesic(x, v)
            results['geodesic'] = curve
        ...
        if return_diagnostics:
            ...
            if v is not None:
                ...
                # Normalize
                v_perp = F.normalize(v_perp, dim=-1)
                
                # Compute curvature
                K = self.compute_sectional_curvature(x, v, v_perp)
                results['sectional_curvature'] = K
