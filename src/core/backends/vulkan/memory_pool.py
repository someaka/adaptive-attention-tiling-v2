...
class MemoryPool:
    ...
    def __init__(
        self, device: vk.Device, memory_type_index: int, initial_size: int = 1024 * 1024
    ):
        self.device = device
        ...
        self.allocation_stats = {"hits": 0, "misses": 0, "fragmentation": 0.0}

    def allocate(
        self, size: int, alignment: int, memory_properties: vk.MemoryPropertyFlags
    ) -> Tuple[vk.DeviceMemory, int]:
    ...
    def _allocate_memory(
        self, size: int, memory_properties: vk.MemoryPropertyFlags
    ) -> vk.DeviceMemory:
        """Allocate new Vulkan memory."""
        alloc_info = vk.MemoryAllocateInfo(
            allocation_size=size, memory_type_index=self.memory_type_index
        )
