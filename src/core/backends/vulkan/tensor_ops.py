...
class VulkanTensorOps:
    ...
    def __init__(self, device: vk.Device, queue: vk.Queue, queue_family_index: int):
        self.device = device
        self.queue = queue
    ...
    def adaptive_attention(self,
                         ...
                         values: torch.Tensor,
                         resolution: float,
                         density_threshold: float = 0.5) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        config = CommandConfig(
            ...
            descriptor_sets=[self._create_attention_descriptors(
    ...
    def adaptive_attention_backward(self,
                                  grad_output: torch.Tensor,
                                  queries: torch.Tensor,
                                  keys: torch.Tensor,
                                  values: torch.Tensor,
                                  resolution: float,
                                  density_threshold: float = 0.5) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        config = CommandConfig(
            ...
            descriptor_sets=[self._create_attention_backward_descriptors(
                grad_desc, q_desc, k_desc, v_desc,
                grad_q_desc, grad_k_desc, grad_v_desc
            )],
            push_constants=self._create_attention_push_constants(
                seq_len, num_heads, head_dim, tile_size,
                resolution, density_threshold
            ),
            dispatch_x=(batch * num_heads * num_tiles + 255) // 256
        )
        ...
        self.command_manager.record_compute_commands(
            cmd,
            self.pipeline_manager.get_pipeline(PipelineType.ADAPTIVE_ATTENTION_BACKWARD),
    ...
    def _create_attention_push_constants(self,
                                       seq_len: int,
                                       num_heads: int,
                                       head_dim: int,
                                       tile_size: int,
                                       resolution: float,
                                       density_threshold: float) -> bytes:
    ...
    def _create_attention_backward_descriptors(self,
                                             ...
                                             grad_v_desc: TensorDescriptor) -> vk.DescriptorSet:
    ...
    def _copy_to_device(self, tensor: torch.Tensor, desc: TensorDescriptor):
        ...
        data_ptr = vk.map_memory(
            self.device,
            self.memory_manager.get_memory(desc.buffer),
            desc.offset,
            desc.size,
            0
        )
        ...
        vk.unmap_memory(self.device, self.memory_manager.get_memory(desc.buffer))
    ...
    def _create_matmul_descriptors(self,
                                 a_desc: TensorDescriptor,
                                 b_desc: TensorDescriptor,
                                 out_desc: TensorDescriptor) -> vk.DescriptorSet:
    ...
    def _init_pipelines(self):
        ...
        self.pipeline_manager.create_pipeline(
            PipelineType.ADAPTIVE_ATTENTION,
            "shaders/adaptive_attention.comp.spv",
            [(0, vk.DescriptorType.STORAGE_BUFFER),
             (1, vk.DescriptorType.STORAGE_BUFFER),
             (2, vk.DescriptorType.STORAGE_BUFFER),
        ...
        self.pipeline_manager.create_pipeline(
            PipelineType.ADAPTIVE_ATTENTION_BACKWARD,
            "shaders/adaptive_attention_backward.comp.spv",
            [(0, vk.DescriptorType.STORAGE_BUFFER),
             (1, vk.DescriptorType.STORAGE_BUFFER),
             (2, vk.DescriptorType.STORAGE_BUFFER),
             (3, vk.DescriptorType.STORAGE_BUFFER),
