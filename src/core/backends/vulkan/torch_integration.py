...
class VulkanTorchBackend:
    ...
    def __init__(self):
        ...
        self.command_pool_manager = CommandPoolManager(
            self.device.device, self.device.queue_family_indices.compute
        )
    ...
    def register_tensor(self, tensor: torch.Tensor) -> TensorInfo:
        ...
        data_ptr = vk.map_memory(self.device.device, memory, offset, size_bytes, 0)
    ...
    def compute_tile(
        ...
        submit_info = vk.SubmitInfo(command_buffers=[command_buffer])
