...
class VulkanDevice:
    ...
    def _create_logical_device(self):
        ...
        # Get queue handles
        self.compute_queue = vk.get_device_queue(
            self.device, indices.compute, 0
        )
        self.transfer_queue = vk.get_device_queue(
            self.device, indices.transfer, 0
        )
    ...
    def _find_queue_families(self, device: vk.PhysicalDevice) -> QueueFamilyIndices:
        ...
        for i, prop in enumerate(properties):
            ...
            if (prop.queue_flags & vk.QueueFlagBits.COMPUTE and 
                indices.compute is None):
                indices.compute = i
                
            # Look for transfer queue
            if (prop.queue_flags & vk.QueueFlagBits.TRANSFER and 
                indices.transfer is None):
                indices.transfer = i
