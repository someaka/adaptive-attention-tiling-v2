...
class MemoryManager:
    ...
    def _create_memory_pool(self, size: int, memory_type_index: int) -> vk.DeviceMemory:
        ...
        alloc_info = vk.MemoryAllocateInfo(
            allocationSize=size, memoryTypeIndex=memory_type_index
        )
    ...
    def allocate_buffer(
        ...
        buffer_info = vk.BufferCreateInfo(
            size=size, usage=usage, sharingMode=vk.SHARING_MODE_EXCLUSIVE
        )
    ...
    def copy_to_device(
        ...
        staging_buffer, staging_offset = self.allocate_buffer(
            size=data_size, usage=vk.BUFFER_USAGE_TRANSFER_SRC_BIT, device_local=False
        )

        # Map memory and copy data
        memory_ptr = self.device.mapMemory(self.staging_pool, staging_offset, data_size)
    ...
    def copy_from_device(
        ...
        # Allocate staging buffer
        staging_buffer, staging_offset = self.allocate_buffer(
            size=size, usage=vk.BUFFER_USAGE_TRANSFER_DST_BIT, device_local=False
        )
        ...
        memory_ptr = self.device.mapMemory(self.staging_pool, staging_offset, size)
