...
from .crystal import BlochFunction, BravaisLattice
...
class RicciFlow:
    ...
    def __init__(self, hilbert_space: HilbertSpace, flow_time: float = 0.01):
    ...
    def compute_ricci_curvature(self, state: QuantumState) -> torch.Tensor:
        ...
        grad_metric = torch.autograd.grad(
            metric.sum(), state.amplitudes, create_graph=True
        )[0]
    ...
    def flow_step(self, state: QuantumState) -> QuantumState:
        ...
        new_amplitudes = torch.einsum(
            "ij,j->i", self.metric_evolution, state.amplitudes
        )
...
class MeanCurvatureFlow:
    ...
    def __init__(
        self, hilbert_space: HilbertSpace, lattice: Optional[BravaisLattice] = None
    ):
        self.hilbert_space = hilbert_space
    ...
    def flow_step(self, state: QuantumState, dt: float = 0.01) -> QuantumState:
...
class BerryTransport:
    ...
    def compute_berry_phase(self, path: List[QuantumState]) -> torch.Tensor:
        ...
        for i in range(len(path) - 1):
            overlap = torch.sum(path[i].amplitudes.conj() * path[i + 1].amplitudes)
            phase += torch.angle(overlap)

        return phase

    def parallel_transport(
        self, state: QuantumState, tangent: torch.Tensor
    ) -> QuantumState:
...
class GeometricFlowAnalyzer:
    ...
    def analyze_flow(
        self, state: QuantumState, num_steps: int = 100, dt: float = 0.01
        ...
        for _ in range(num_steps):
            ...
            # Mean curvature flow
            mean_curvature = self.mean_curvature.compute_mean_curvature(current_state)
            mc_evolved = self.mean_curvature.flow_step(current_state, dt)

            # Geometric phase
            berry_phase = self.berry.compute_berry_phase([current_state, mc_evolved])

            # Holonomy
            tangent = mc_evolved.amplitudes - current_state.amplitudes
            transported = self.berry.parallel_transport(current_state, tangent)
            holonomy = torch.sum(transported.amplitudes.conj() * mc_evolved.amplitudes)

            metrics.append(
                GeometricFlowMetrics(
