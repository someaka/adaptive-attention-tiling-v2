...
class ActionFunctional:
    ...
    def __init__(
        ...
        potential_rank: int = 4
    ):
        self.hilbert_space = hilbert_space
        
        # Kinetic and potential terms
        self.kinetic = nn.Parameter(
    ...
    def stationary_points(
        ...
        path = torch.linspace(0, 1, num_points).unsqueeze(-1) @ \
               (end - start).unsqueeze(0) + start
...
class Propagator:
    ...
    def propagate(
        ...
        propagator = torch.sum(weights.unsqueeze(-1) * \
                             torch.stack([p.points[-1] for p in all_paths]), dim=0)
...
class StationaryPhase:
    ...
    def quantum_corrections(
        ...
        hessian = torch.stack([
            torch.autograd.grad(g, points, retain_graph=True)[0]
            for g in grad
        ])
