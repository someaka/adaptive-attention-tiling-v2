...
def get_hardware_profile() -> HardwareProfile:
    ...
    if has_vulkan:
        device_name = 'vulkan'
        device_capabilities = {'memory': available_memory_gb}  # Use system memory as proxy until Vulkan API provides this
    else:
        device_name = 'cpu'
...
def get_safe_model_config(profile: HardwareProfile) -> Dict[str, int]:
    ...
    def estimate_memory_gb(config: Dict[str, int]) -> float:
        ...
        num_params = (4 * config['hidden_dim'] * config['hidden_dim']) + (2 * config['hidden_dim'])  # Linear layers + biases

        total_elements = (input_size + key_query_value_size + attention_size + output_size + num_params) * overhead_factor
    ...
    if estimated_full < safe_memory:
        return full_config
    elif estimated_medium < safe_memory:
        return medium_config
    elif estimated_min < safe_memory_min:
        return min_config
    else:
