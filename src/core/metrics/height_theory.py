...
class HeightStructure:
    ...
    def __init__(
        self, num_primes: int = 8, base_field: str = "real"  # 'real' or 'p-adic'
    ):
    ...
    def compute_local_height(self, point: torch.Tensor, prime: int) -> torch.Tensor:
    ...
    def compute_canonical_height(self, point: torch.Tensor) -> torch.Tensor:
    ...
    def analyze_growth(
        self, points: List[torch.Tensor], window_size: int = 10
    ) -> Dict[str, torch.Tensor]:
        """Analyze height growth over sequence of points."""
        heights = torch.stack([self.compute_canonical_height(p) for p in points])
...
class AdaptiveHeightTheory:
    ...
    def __init__(
        self, hidden_dim: int, num_primes: int = 8, learning_rate: float = 0.001
    ):
        ...
        self.optimizer = torch.optim.Adam(
            self.height_projection.parameters(), lr=learning_rate
        )
