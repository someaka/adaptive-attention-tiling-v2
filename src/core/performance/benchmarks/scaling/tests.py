...
class ScalingBenchmarks:
    ...
    def __init__(self, base_size: int = 1024, max_scale: int = 8, device: str = "cpu"):
    ...
    def benchmark_memory_scaling(
        self, base_size: Optional[int] = None
    ) -> List[ScalingResult]:
        ...
        for scale in range(1, self.max_scale + 1):
            ...
            for _ in range(scale):
                if self.memory_manager:
                    tensor = self.memory_manager.pool.acquire(
                        (size, size), torch.float32
                    )
                else:
                    tensor = torch.randn(size, size, device=self.device)
    ...
    def run_all_benchmarks(self) -> Dict[str, List[ScalingResult]]:
        ...
        return {
            "strong": self.benchmark_strong_scaling(
                test_computation, (self.base_size, self.base_size)
            ),
    ...
    def get_summary(self) -> Dict[str, Dict[str, float]]:
        ...
        for result in self.results:
            if result.test_type not in summary:
                ...
                stats["avg_speedup"] = (
                    stats["avg_speedup"] * stats["count"] + result.speedup
                ) / (stats["count"] + 1)
                stats["avg_efficiency"] = (
                    stats["avg_efficiency"] * stats["count"] + result.efficiency
                ) / (stats["count"] + 1)
                stats["avg_memory_per_unit"] = (
                    stats["avg_memory_per_unit"] * stats["count"]
                    + result.memory_per_unit
                ) / (stats["count"] + 1)
