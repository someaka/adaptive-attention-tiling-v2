...
import numpy as np
from ...cpu.memory import MemoryManager, MemoryPool, CacheOptimizer
...
class MemoryBenchmarks:
    ...
    def benchmark_allocation_patterns(self,
                                   sizes: List[int] = [1024, 4096, 16384, 65536],
                                   dtype: torch.dtype = torch.float32) -> List[MemoryBenchmarkResult]:
    ...
    def benchmark_transfer_speeds(self,
                                sizes: List[int] = [1024, 4096, 16384, 65536],
                                dtype: torch.dtype = torch.float32) -> List[MemoryBenchmarkResult]:
    ...
    def benchmark_cache_efficiency(self,
        ...
        for size in sizes:
            ...
            for _ in range(self.runs_per_test):
                ...
                cache_misses.append(0)  # Simplified - would need HW counters for real values
    ...
    def get_summary(self) -> Dict[str, Dict[str, float]]:
        ...
        for result in self.results:
            if result.operation not in summary:
                ...
                stats['avg_data_size'] = (stats['avg_data_size'] * stats['count'] + 
                                        result.data_size) / (stats['count'] + 1)
                if result.allocation_time:
                    stats['avg_allocation_time'] = (stats['avg_allocation_time'] * 
                                                  stats['count'] + 
                                                  result.allocation_time) / (stats['count'] + 1)
                if result.transfer_time:
                    stats['avg_transfer_time'] = (stats['avg_transfer_time'] * 
                                                stats['count'] + 
                                                result.transfer_time) / (stats['count'] + 1)
                if result.bandwidth:
                    stats['avg_bandwidth'] = (stats['avg_bandwidth'] * stats['count'] + 
                                            result.bandwidth) / (stats['count'] + 1)
                if result.cache_hits:
                    stats['avg_cache_hits'] = (stats['avg_cache_hits'] * stats['count'] + 
                                             result.cache_hits) / (stats['count'] + 1)
                stats['avg_fragmentation'] = (stats['avg_fragmentation'] * stats['count'] + 
                                            result.fragmentation) / (stats['count'] + 1)
                stats['count'] += 1
        return summary
