...
class BufferManager:
    ...
    def create_buffer(
        ...
        buffer_info = BufferInfo(
            buffer=buffer, memory=memory, size=size, usage=usage, properties=properties
        )
    ...
    def map_memory(self, buffer_info: BufferInfo) -> int:
        ...
        if not buffer_info.is_mapped:
            buffer_info.mapped_ptr = vk.MapMemory(
                self.device, buffer_info.memory, 0, buffer_info.size, 0
            )
    ...
    def copy_to_device(
        ...
        try:
            ...
            copy_region = vk.BufferCopy(srcOffset=0, dstOffset=0, size=data_size)
            vk.CmdCopyBuffer(
                command_buffer,
