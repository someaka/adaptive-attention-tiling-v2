...
class GPUMemoryManager:
    ...
    def __init__(self, device_id: int = 0):
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is not available. GPU operations cannot be performed.")
    ...
    def allocate_tensor(self, size: Tuple[int, ...], dtype: torch.dtype = torch.float32) -> torch.Tensor:
        ...
        # Record metrics
        self._metrics.append(GPUMemoryMetrics(
    ...
    def get_cache_memory(self) -> int:
        ...
        return torch.cuda.memory_reserved(self._device_id) - torch.cuda.memory_allocated(self._device_id)
