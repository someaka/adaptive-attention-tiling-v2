...
class MetricsCollector:
    ...
    def record_metric(self, 
                     operation_type: str,
                     execution_time: float,
                     cache_stats: Optional[Dict[str, int]] = None) -> None:
    ...
    def get_metrics(self, 
                   ...
                   time_range: Optional[tuple[float, float]] = None) -> List[PerformanceMetrics]:
        """Get metrics, optionally filtered by operation type and time range."""
        metrics = self.metrics[operation_type] if operation_type else \
                 [m for metrics in self.metrics.values() for m in metrics]
    ...
    def get_summary(self, 
                   operation_type: Optional[str] = None) -> Dict[str, float]:
    ...
    def export_metrics(self, format: str = 'dict') -> Union[Dict, str]:
        ...
        if format == 'dict':
            return {op: [vars(m) for m in metrics] 
                   for op, metrics in self.metrics.items()}
        elif format == 'csv':
            # Create CSV string
            headers = ['operation_type', 'execution_time', 'cpu_usage', 
                      'memory_usage', 'cache_hits', 'cache_misses', 'timestamp']
            rows = [','.join(headers)]
